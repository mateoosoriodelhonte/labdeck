...
The validated manifest plan can contain environment values. Its full fingerprint depends on
...
Store test lab revision, service, nonsecret test-plan digest, timestamp, status, outcome reason,
duration, optional exit code, and separate standard output and error.
...
runner must give the sanitizer all known sensitive environment values. Retain at most 100 test
...
### Persist the full manifest fingerprint

The fingerprint can detect a changed manifest. It also depends on environment values and could make
a guessed credential testable. It is not stored. The assignment runner stores a separate digest of
only the test service, timeout, and argv.
